...
Use the Ubuntu 24.04 amd64 `.deb` described in the root
[installation guide](../README.md#installation). It installs AcornFS and the
Nautilus integration together through normal system package dependencies:
...
The package installs `/usr/bin/acornfs`, a Nautilus bootstrap, the shared MIME
package and hidden desktop handler below `/usr`. Package upgrades and removal
preserve images, preferences, checkpoints and repair audits. The standalone
per-user add-on remains an alternative when system package installation is not
available.
...
For a system package installation, unmount every image, confirm `acornfs status`
is empty, then run:
...
For the alternative add-on, use `python3 install.py --restart uninstall` from
its extracted directory. For an editable development installation, run
`acornfs uninstall-nautilus --restart`. All paths retain images, preferences,
recovery checkpoints and repair audits.
