...
was inspected or mounted. Preserve the original image and any companion DSC
before recovery work. Do not substitute a descriptor from another image because
its filename matches: cylinder and head geometry determines how its DAT is
addressed.

## Assessment workflow

1. Copy the original image and any companion file to read-only archival storage.
...
5. If the full plan is marked applicable, run `acornfs repair IMAGE --confirm
   DAT_FILENAME` only on the working copy.
6. Mount read-only only when AcornFS can safely traverse the full directory
...
recovery checkpoint before the first mutation, verifies the full image
