...
def _normalise_stem(name: str) -> str:
    ...
    if path.name != candidate or candidate in {".", ".."}:
        raise AcornFSError("The image name must be a filename, not a path.")
    if path.suffix.casefold() in {".dat", ".dsc"}:
        candidate = path.stem
    if not _SAFE_STEM.fullmatch(candidate):
        raise AcornFSError(
            "The image name must contain 1–64 ASCII letters, digits, dots, dashes or underscores."
        )
    return candidate
...
def _normalise_title(title: str) -> str:
    ...
    try:
        ...
        raise AcornFSError("The ADFS title must contain ASCII characters only.") from exc
    if len(encoded) > 12 or any(value < 0x20 or value == 0x7F for value in encoded):
        raise AcornFSError("The ADFS title must contain 1–12 printable ASCII characters.")
    return candidate
...
def create_beebscsi_image(
    ...
    *,
    ...
    report_progress(progress, 0, "Checking image settings…")
    parent = Path(directory).expanduser().resolve()
    if not parent.is_dir():
        raise AcornFSError(f"The image destination is not a directory: {parent}")
    ...
    try:
        ...
        raise AcornFSError(f"Could not inspect the destination directory: {exc}") from exc
    if collisions:
        raise AcornFSError(f"Image creation would overwrite existing file: {collisions[0]}")

    filesystem = create_filesystem("adfs")
    try:
        geometry = filesystem.geometry_grammar().parse(f"capacity={capacity.strip() or '20MB'}")
    except Exception as exc:
        raise AcornFSError(f"Invalid BeebSCSI capacity: {exc}") from exc
    if (
        ...
        raise AcornFSError(
            "The requested capacity exceeds BeebSCSI's 21-bit ADFS, 16-head or DSC limits."
        )
    ...
    published: list[Path] = []
    try:
        report_progress(progress, 10, "Creating empty old-format ADFS filesystem…")
        try:
            filesystem.create(temporary_dat, geometry, title=disc_title)
        except Exception as exc:
            raise AcornFSError(f"Could not create the temporary ADFS filesystem: {exc}") from exc
        if not temporary_dsc.is_file():
            raise AcornFSError("The filesystem engine did not create a matching DSC descriptor.")
        report_progress(progress, 70, "Validating the complete DAT/DSC pair…")
        report = validate_image_report(temporary_dat)
        if report.fatal_findings:
            first = report.fatal_findings[0]
            raise AcornFSError(f"Created image failed validation: {first.code}: {first.message}")

        report_progress(progress, 90, "Publishing the validated image pair…")
        try:
            ...
            _sync_directory(parent)
            raise AcornFSError(
                f"Could not publish the image pair without overwriting: {exc}"
            ) from exc
    finally:
    ...
    report_progress(progress, 100, "BeebSCSI image created and verified")
